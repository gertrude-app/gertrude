...
struct StripeBillingOverhaul: GertieMigration {
  func up(sql: SQLDatabase) async throws {
    ...
    try await sql.execute("""
      ...
      FROM (VALUES
        ...
        ('sub_1TU8QTGKRdhETuKAnaB2GMnh', 'cus_UT4ZqbPliY69OJ'),
        ('sub_1TUWS9GKRdhETuKAfFU2ZGsC', 'cus_UTTOeq4CuHjBDA'),
        ('sub_1TV3zvGKRdhETuKAe4WIEUNt', 'cus_UU245umSb4HxN1'),
        ('sub_1TVJ4IGKRdhETuKAFuqdCWmi', 'cus_UUHdNTk0R5efZE'),
        ('sub_1TW5tQGKRdhETuKAEktTyHJW', 'cus_UV65Ikq6j9Un32'),
        ('sub_1TWLLSGKRdhETuKA0WhbBm6V', 'cus_UVM3Soqeid1SGT'),
        ('sub_1TWQFqGKRdhETuKAcG4anNm2', 'cus_UVR8r6BtTf5WVB'),
        ('sub_1TXke0GKRdhETuKAWJBOw5hu', 'cus_UWoGf3Pqhbzmlk')
      ) AS m(sub_id, cus_id)
    ...
    try await sql.execute("""
      ...
      BEGIN
        IF EXISTS (
          ...
          RAISE EXCEPTION 'gap detected: identity rows with last_stripe_subscription_id but no stripe_customer_id — regenerate VALUES block and redeploy';
        END IF;
      END $$;
    """)
  ...
  // NB: not fully reversible, good enough for unit tests
