...
struct GetIOSDevice_v3: Pair {
  ...
  struct Output: PairOutput {
    ...
    struct MusicInstall: PairNestable {
      var subscription: MusicSubscriptionState
    }
...
extension GetIOSDevice_v3: Resolver {
  ...
  static func musicInstall(for device: IOSDevice, in ctx: ParentContext) async throws -> Output
    ...
    return Output.MusicInstall(
      subscription: account.musicSubscriptionState(for: token),
    )
  }
}
