...
extension StartCheckoutSession: Resolver {
  static func resolve(with input: Input, in context: ParentContext) async throws -> Output {
    ...
    if let sub = account.stripeSubscription, sub.stripeStatus.isLive {
      throw context.error(
        "20de8a91",
        .badRequest,
    ...
    let customerId = account.stripeCustomerId?.rawValue
    ...
    guard let url = session.url else {
      with(dependency: \.postmark)
        .unexpected("3b4f9a5d", "parent: \(context.parent.id)")
