...
/// Deterministic seeded interleaving explorer for the Mac app ↔ filter
/// system-extension protocol, ported from the iOS blocker's
/// `RecordingExplorer` (see `docs/ios-conformance.md` for the pattern). Each
/// run drives a fresh `VirtualMac` through a random-but-reproducible sequence
/// of user, parent, OS, and adversarial events — flows, time, suspension
/// grants, rules changes, downtime edges, app launch/quit, filter
/// kill/respawn/crash-recovery, reboots, and two macOS users fighting over the
/// single retained XPC connection (OS RULE M5) — checking after every
/// observable flow that the real processes' verdicts match an independent
/// spec oracle, and at the end of the run that the world converged.
///
/// The Mac has no shared registers (state converges only through XPC), so
/// unlike the iOS oracle (stateless math over durable registers), the Mac
/// oracle is a state machine replayed over the *observable message log*: only
/// app→filter messages that actually REACHED the filter (`world.deliveredXpc`)
/// update its model, plus the witnessed OS lifecycle rules (M1 respawn reloads
/// disk, M3 fail-open, M4 verdict finality). Intent that never arrived — an
/// XPC send that failed because another user held the connection, or the
/// provider was dead — must not move the spec's model, exactly as it cannot
/// move the real filter.
enum MacExplorer {}

/// SplitMix64: tiny, high-quality, and stable across platforms/releases —
/// the same seed must reproduce the same run forever, since failing seeds
...
/// the two onboarded macOS users of the simulated Mac (OS RULE M5: only one
/// app at a time holds the filter's retained XPC connection, so two users is
/// the minimal universe that exercises the last-wins churn)
...
/// The spec's model of the filter process, replayed from the delivered
/// message log + OS lifecycle rules. Mirrors what a correct filter must be
/// equivalent to: durable state (keychains, downtime window) reloaded from
/// disk on every (re)spawn per OS RULE M1; ephemeral state (app-liveness,
/// suspensions, downtime pause) lost with the process. Verdict derivation
/// re-implements the `Decision+Early`/`Decision+Flow` semantics for the
/// explorer's restricted universe (Safari flows from uids ≥ 501, domain keys
/// with unrestricted scope, no exemption/filtering-disabled/always-blocked).
struct MacOracle: Sendable {
  struct UserMemory: Sendable {
    /// nil = filter knows nothing about this user (unprotected: every flow
    /// drops with noUserKeys for uid ≥ 501 unless suspended/failed-open)
  ...
  enum AliveEntry {
    case present
    case absent
    /// alive lapsed less than one filter-heartbeat interval ago: the entry
    /// may or may not have been reaped yet — verdicts that depend on it are
    /// indeterminate and skipped (counted in stats)
    case boundary
  }
  ...
  /// in-memory state (liveness, suspensions, downtime pause) starts empty
  ...
  /// replays one delivered app→filter message, mirroring the state effects of
  /// `Filter.reduce` (incl. `recordAppActivity`'s alive refresh on every
  /// user-scoped message) and what `state.persistent` durably saves
  mutating func consume(_ message: DeliveredXPCMessage) {
    var m = self.memory[message.uid] ?? UserMemory()
    switch message.request {
    case .alive, .ackRequest:
      m.aliveUntil = message.at + 150
  ...
  /// the filter reaps a lapsed alive entry only at its next 60s heartbeat
  /// tick, so within one interval of the lapse the entry's presence is
  /// indeterminate; a lapse older than a full interval must have been reaped
  func aliveEntry(for uid: uid_t, now: Date) -> AliveEntry {
    ...
    if until < now - 60 { return .absent }
    return .boundary
  }

  func expectedVerdict(uid: uid_t, host: String, now: Date, calendar: Calendar) -> Verdict {
    // OS RULE M3: no provider process → all traffic flows unfiltered
  ...
  /// the oracle's own re-derivation of which hostnames a delivered keychain
  /// set allows — deliberately NOT the production `KeychainIndex` matcher, so
  /// a matching bug can't hide in both places. Restricted to the explorer's
  /// key universe: exact-domain keys with unrestricted scope, no schedules.
...
extension MacExplorer {
  ...
  /// deterministic, so this reliably turns a fuzz run into a minimal
  /// scenario a human can read.
  @MainActor
  static func shrink(
...
final class MacExplorerRun {
  ...
  /// feeds every app→filter message that reached the filter since the last
  ...
  /// convergence: with no further user events, any reachable state must
  /// settle — after time passes, a reboot, and users logging back in, the
  /// world and the oracle must agree on every verdict, no suspension may
  /// survive anywhere, the XPC connection must be re-established, and
  /// pre-reboot connections must be dead (OS RULE M4)
  func converge(step: Int) async -> MacExplorer.Violation? {
    ...
    if self.world.retainedConnectionUid.value == nil {
      return self.violation(
        "C-mac-xpc-not-reestablished", step, action,
        "no app holds the filter's retained XPC connection after relaunch",
      )
    }
