...
// Intent-level claim: assign the pending music device to a NEW child through the real
...
describe(`Gertrude Music device claim`, () => {
  it(`claims the pending device to a new child and reaches the connected screen`, () => {
    cy.visit(`/claim-music-device/${code}/claim`, { onBeforeLoad: seedAuth });
    ...
    cy.url().should(`include`, `/claim-music-device/${code}/done`); // persisted + advanced
