...
export function subtext(input: Input): string {
  ...
  switch (entitlement.case) {
    ...
    case `fullTrial`:
      return `Your free trial ends ${long(entitlement.until)}. Subscribe to keep using Gertrude across all your family’s devices.`;
    case `fullTrialGrace`:
      return `Your Full trial has ended. Subscribe before the grace period ends on ${long(entitlement.until)} to avoid diminished safety on protected Mac computers.`;
  }
}
...
export function manageBlurb(input: Input): string | undefined {
  const { entitlement, billing, fullTrialStartedAt, lastPaidTier } = input;
  switch (entitlement.case) {
    case `complimentary`:
      return undefined;

    case `full`: {
      const { status, currentPeriodEnd } = billing;
  ...
  switch (action.case) {
    case `startCheckout`: {
      const isFreeBrandNew =
        input.entitlement.case === `free` && !input.fullTrialStartedAt;
...
export function badge(input: Input): BadgeOutput {
  const { entitlement, billing } = input;
  switch (entitlement.case) {
    case `complimentary`:
      return { text: `Complimentary`, type: `ok` };
    case `fullTrial`: {
      const msUntilTrialEnd = new Date(entitlement.until).getTime() - Date.now();
    ...
    case `light`:
    case `full`:
      if (billing.status === `pastDue`) return { text: `Past due`, type: `red` };
