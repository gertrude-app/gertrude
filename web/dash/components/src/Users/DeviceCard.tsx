import cx from 'classnames';
import React from 'react';
import { Link } from 'react-router-dom';
import type { ChildComputerStatus } from '@dash/types';
import UserStatus from '../UserStatus';

export type DeviceCardStatus =
  | { case: `computerStatus`; status: ChildComputerStatus }
  | { case: `pendingSetup` };

type Props = {
  to: string;
  imageSrc: string;
  imageAlt: string;
  title: string;
  subtitle?: string;
  status?: DeviceCardStatus;
  className?: string;
};

const DeviceCard: React.FC<Props> = ({
  to,
  imageSrc,
  imageAlt,
  title,
  subtitle,
  status,
  className,
}) => (
  <Link
    to={to}
    className={cx(
      `rounded-2xl p-2 px-2 xs:px-3 bg-white border border-slate-200 transition-[background-color] duration-100 hover:bg-slate-50 flex justify-between items-center gap-6 @container`,
      className,
    )}
  >
    <div className="flex items-center gap-3 xs:gap-4 sm:gap-6">
      <div className="rounded-full bg-slate-100 w-8 xs:w-14 h-8 xs:h-14 shrink-0 flex justify-center items-center">
        <img
          src={imageSrc}
          alt={imageAlt}
          className="max-w-[60%] max-h-[60%] grayscale-[50%]"
        />
      </div>
      <div>
        <h3 className="text-slate-900 xs:text-lg font-bold line-clamp-1 -mb-0.5 xs:mb-0">
          {title}
        </h3>
        {subtitle && <h4 className="text-sm text-slate-500 line-clamp-1">{subtitle}</h4>}
      </div>
    </div>
    {status?.case === `computerStatus` && <UserStatus status={status.status} />}
    {status?.case === `pendingSetup` && (
      <span className="shrink-0 text-xs font-medium text-amber-700 bg-amber-50 border border-amber-200 rounded-full px-3.5 py-2">
        <i className="fa-solid fa-exclamation-triangle mr-1.5" />
        Finish setup <i className="fa-solid fa-arrow-right ml-1" />
      </span>
    )}
  </Link>
);

export default DeviceCard;
