...
The app sends a small amount of anonymous app data to help us understand usage patterns,
keep subscriptions working, and fix bugs. This can include:
...
- App events, such as first launch, onboarding completion, subscription status changes,
  review prompt choices, and error reports.
...
advertising. Once a device is connected to a Gertrude account, this data is associated with
that account.
...
complimentary access is active. Legacy subscriptions purchased through Apple before version
1.6.0 are honored during a migration period.

### Support Actions

The app includes support-only actions that can upload or download the app's local database
when troubleshooting requires it. These actions are not automatic; they require a special
command entered into the app.
