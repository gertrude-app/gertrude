...
description:
  A step-by-step guide to completely locking down an iPhone or iPad for iOS 17, for
...
{% ios-version-picker current="ios-17" /%}
...
[major loophole](/help/iphone-ipad/block-gif-search-in-messages) in its parental controls:
there was no longer any way to block children from searching for animated GIFs, including
...
2. The free [Gertrude Blocker](/help/iphone-ipad/block-gif-search-in-messages) iOS app we
   released.

If you'd like to close this loophole with Gertrude Blocker,
[click here](/help/iphone-ipad/block-gif-search-in-messages) for detailed setup
instructions. Once you've got the app installed, continue on with the tutorial from this
point.

If you don't want to use Gertrude Blocker, take note of the section labeled "Blocking
...
will be able to
[search for and view images and content](/guides/iphone-lockdown-loopholes) from the
internet through their phone's search and look up features.
...
with a passcode&mdash;[do that first](#step-4-set-a-screen-time-passcode).
...
If you didn't install [Gertrude Blocker](/help/iphone-ipad/block-gif-search-in-messages),
you should use a workaround with Downtime to block the #images GIF search in the built-in
texting app. Set up an every-day scheduled Downtime to be _always on_ by making it go from
**3:00AM** to **2:59AM**. This will block the #images GIF search, but will require that
you list every app you want your child to use as "Always Allowed" which is described in
the section below. This basically prevents you from using the Downtime feature as
intended, but the "App Limits" feature can still be used to limit app usage.
...
Remember the apps in [Step 1](#step-1-delete-apps) that Apple wouldn't let you delete?
...
Next, tap the back button to get back to the _Content & Privacy Restrictions_ screen, and
then tap to move into the third section, labeled **Store, Web, Siri & Game Center
Content**:
...
Within the same _Store, Web, Siri & Game Center_ section there is another section labeled
**Apps**, as shown below:
...
[things you probably forgot to lock down](/guides/iphone-lockdown-loopholes).
