...
{% callout type="warning" title="These setup instructions are outdated" %}

Gertrude Blocker still blocks GIF search in Messages, but its current setup flow differs
from the screenshots and steps below. We’re working on updated instructions. For now,
install or open Gertrude Blocker and follow its on-screen setup.
...
[new holes in Screen Time](/help/iphone-ipad/block-web-content-in-device-search) were
found, including the inability for parents to restrict image searches through Spotlight.
So we ended up taking matters into our own hands and built
