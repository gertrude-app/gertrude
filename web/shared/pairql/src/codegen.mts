...
async function generateDomain(config: DomainConfig): Promise<void> {
  ...
  if (config.contractHashFile) {
    if (!output.contractHash) {
      throw new Error(`Missing ${config.domain} contract hash`);
    }
    fs.writeFileSync(
      path.join(domainDir, `contract.ts`),
      autogenerated(
        `export const DASHBOARD_PAIRQL_CONTRACT_HASH = \`${output.contractHash}\`;\n`,
      ),
    );
    pairIndexLines.push(`export * from './contract';`);
  }
  ...
  if (sharedTypes.length > 0) {
    ...
    sharedFile = `export type { ServerPqlError } from '../PqlError';\n\n` + sharedFile;
  ...
  for (const [pairName, pair] of Object.entries(output.pairs).sort(([a], [b]) =>
    ...
    const importBlock =
      sharedImports.length > 0
        ? `import type { ${sharedImports.join(`, `)} } from '../shared';\n`
        : ``;
    fs.writeFileSync(
