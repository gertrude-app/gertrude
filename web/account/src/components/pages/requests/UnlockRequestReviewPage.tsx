...
const AppRequestSection: React.FC<{
  ...
  iconUrl?: string;
  rowStates: Map<string, UnlockRowState>;
  ...
  const decisionCounts = entry.groups.reduce(
    (counts, group) => {
      const row = rowStates.get(group.id);
      const covered = row && !row.problem ? row.coveredRequestCount : 0;
      counts.covered += covered;
      counts[row?.decision ?? group.decision] += group.requestIds.length - covered;
      return counts;
    },
        ...
        >
          {entry.groups.map((group) => (
            <UnlockRequestCard
              ...
              rowState={rowStates.get(group.id)}
...
> = ({ data, saving, appIconUrl, onSubmit, onRefresh, entries, setEntries }) => {
  ...
  React.useEffect(() => {
    const interval = setInterval(() => setReviewTime(Date.now()), 30_000);
    return () => clearInterval(interval);
  }, []);
  const plan = planUnlockReview(entries, reviewTime);
