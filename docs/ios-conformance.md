...
| R10 | broadcast picker launches recorder; suspend sentinel arrives as a flow; screenshots follow suspension; stopping recording resumes filter on next flow; ~50MB memory cap; never auto-relaunched; screen off/locked pauses buffer delivery entirely (liveness gaps are normal) | `recorder-start` → `filter-sentinel suspend-filter`; `filter-suspended` → `recorder-screenshot-saved`; `recorder-stop` → `filter-resumed`; `recorder-liveness-bump` gaps | request + grant suspension, start recording in app, browse, lock the screen ~15s mid-recording, browse again, stop recording, browse again |
...
| R10 | 2026-07-04 | 18.x | hinge session (28.6-min recording, `witnesses-20260704-121535.ndjson`): recorder launch/stop, suspend sentinel (fan-out x7), 292 saves, tombstone all confirmed. NEW: 3 bump gaps of 10-19s from screen off/locked — ReplayKit stops buffer delivery entirely; one gap tripped the pre-D2 one-way suspension exit (fixed: level-triggered re-entry). Liveness propagation: 0ms staleness across 215 filter reads |
| R11 | 2026-07-04 | 18.x | hinge session: zero darwin deliveries to the suspended app during the full recording; `broadcastFinished` arrived only on reopen — confirms held+coalesced-on-resume, exact coalescing latency ≈ resume time |
