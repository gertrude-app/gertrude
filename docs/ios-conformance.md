...
| R13 | flow verdicts are final per socket: an allowed connection never re-consults the filter; long-lived connections opened during a suspension keep carrying data after blocking resumes, until the socket dies (reboot/network transition) | — (un-witnessable directly: the filter is never consulted again; verify by absence — the leaking app keeps functioning with zero new flow-decision witnesses for it) | start a suspension, open YouTube (or stream any video), end the recording, observe continued partial function alongside blocked NEW navigation |
| R14 | shield state written to a named `ManagedSettingsStore` is device policy: applied ≤36ms from controller context, persists writer death AND reboot until overwritten or authorization revoked; `.all(except:)` shields all third-party + categorized first-party apps but exempts Phone/Settings/Messages/Safari/Maps/Find My/Clock; only authorized processes can write | `controller-shield-write` / `app-shield-write` (+ `--shields` analysis pairing sentinel→write latency) | shields-spike runbook (`conformance/shields-spike.md`): sentinel-driven writes, reboot with shields up, revocation; still owed — post-boot ~2s window probe, whether `.all(except:)` shields Gertrude itself |
| R15 | a shielded app is unusable: user-initiated flows and socket use from it do not happen (shield covers the UI instantly; in-progress background audio dies); background-refresh traffic may still flow | — (verify by absence: zero flow-decision witnesses for the shielded app while its shield is up) | with shields raised, attempt to use a shielded app; play audio from a shielded app, raise shields, observe it die; still owed — background-refresh traffic from a shielded app observed at the filter |
...
| R13 | 2025 (spring) | — | observed during the original recording experiment: YouTube remained moderately functional after suspension ended (persistent HTTP/2/QUIC connections; the leak that killed the feature). Never verified with witness tooling — rerun the provocation during the shields conformance spike |
| R14 | 2026-07-05 | 26.5.1 | session 1 (`witnesses-20260704-163920.ndjson`): 75/75 controller-context writes, 0-26ms, sentinel→applied ≤36ms; persists reboot; revocation cleared store silently; `.all(except:)` exemption class observed. Session 2 (`witnesses-20260705-075826.ndjson`): +106/106 writes (2-40ms sentinel→write) incl. mid-suspension; post-boot "unshielded window" RESOLVED as UI lag (tap inside it opened restricted — enforcement continuous); cross-context clears work BOTH directions (app↔controller = same effective store; two-writer design valid); cross-process store READ-BACK stale (app read apps=0 after controller write while shields visibly up — never gate on read-backs); `.all(except:)` EXEMPTS the managing app itself (Gertrude launchable with shields-all up, both writer contexts — human-observed, no witness line exists for shield visibility). OWED: child + supervised-adult auth paths |
| R15 | 2026-07-05 | 26.5.1 | session 1: shield visibility behaviorally instant; WhatsApp background audio died the moment shields rose, no auto-resume. Session 2 CORRECTION: audio-kill is app-specific — already-playing Safari/web audio survives webDomains shields, Safari-by-token shields, and the restricted screen itself (dies only on page re-render); bounded passive leak. OWED: flow-absence verification for a shielded app, background-refresh traffic observation (sim models it flowing — unverified), exit-starvation timing (needs integrated feature) |
